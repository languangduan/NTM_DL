...
def build_new_tag_for_25actegory(doc_length=None,file_path='wiki10+_tag-data/tag-data.xml',train_out_filepath='new_dataset_for_25_top_cat_train.json',test_out_filepath='new_dataset_for_25_top_cat_test.json',choosen_cate_number=25,split_proportion=4):
    tag_tree = ET.parse(file_path)
    root = tag_tree.getroot()
    category = get_categories(root)
    sorted_category = sorted(category.items(), key=operator.itemgetter(1),reverse=True)
    ...
    if doc_length:
        hash_categories_pair=hash_categories_pair[:doc_length]
    else:
        pass
    ...
    train_dataset['hash_categories_pair']=hash_categories_pair[:int(split_proportion*l/(split_proportion+1))]
    test_dataset['hash_categories_pair']=hash_categories_pair[int(split_proportion*l/(split_proportion+1)):]
...
if __name__ == "__main__":
    ...
    build_new_tag_for_25actegory(doc_length=120,file_path='wiki10+_tag-data/tag-data.xml',choosen_cate_number=25,split_proportion=4)
